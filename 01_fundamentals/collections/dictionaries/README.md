...
# 91. Dictionary Methods Cheat Sheet
...
| `setdefault()` | Get value and create key if missing |
...
# 92. Dictionary Operations Cheat Sheet
...
Find the employee earning 3500:
...
The `None` is returned if no match exists.

---

# 95. Find Multiple Keys by Value
...
# 96. First Matching Dictionary Entry

Suppose:

```python
employees = [
    {"name": "Ivan", "salary": 2800},
    {"name": "Maria", "salary": 3200},
    {"name": "Georgi", "salary": 3500}
]
```

Find the first employee earning over 3000:
...
This is useful for finding the first matching item without processing the entire collection.

---

# 98. Dictionary and JSON

Dictionaries are heavily used when working with JSON.
...
If the JSON contains an object, `data` will typically be a Python dictionary.

---

# 102. Writing Dictionary to JSON File
...
# 103. Dictionary vs JSON

A Python dictionary and JSON object are related, but they are **not the same thing**.
...
The `json` module converts between them.
...
# 104. Dictionary with Mixed Nested Structures
...
Access project hours:

```python
employee["projects"][0]["hours"]
```
...
# 105. Traversing Complex Nested Data
...
# 106. Dictionary Data Cleaning Pattern

Real data may contain missing values.
...
```python
salary = employee.get("salary") or 0
```

Be careful with the second approach because it also replaces other falsy values such as:
...
Use explicit `is None` when that distinction matters.

---

# 107. `dict.get()` and Falsy Values
...
If you need to distinguish between a missing key and a key containing `None`, use:
...
```text
key exists and its value is None
```

---

# 108. Missing Key vs Existing `None`
...
This distinction is important when processing real-world data.

---

# 109. Updating a Dictionary While Iterating

Be careful when changing the **size** of a dictionary while iterating over it.
...
Instead, create a separate list of keys:
...
# 110. Modifying Values While Iterating

Changing existing values is generally fine because the dictionary size doesn't change.
...
The keys remain the same; only values are modified.

---

# 111. Dictionary Comprehension for Filtering
...
# 112. Dictionary Views Are Dynamic

`keys()`, `values()`, and `items()` return **view objects**, not ordinary lists.
...
# 113. Dictionary Views and Membership

You can directly test membership:

```python
"a" in data.keys()
```

But this is normally unnecessary.
...
# 114. Dictionary Items Behave Like Pairs
...
This can be useful for checking whether a specific key-value pair exists.

---

# 115. Dictionary Performance — Basic Understanding
...
| `data.pop(key)`           |               O(1) |
| `len(data)`               |               O(1) |
| Iterating over dictionary |               O(n) |
| `value in data.values()`  |               O(n) |
| `data.items()` iteration  |               O(n) |

These are **average-case** complexities.

This is one reason dictionaries are so useful for lookup operations.
...
is typically much faster for large collections than:
...
when `user_list` is a list of values.

---

# 116. Dictionary vs List for Lookups
...
requires scanning the list in the worst case.

A dictionary can provide direct key lookup:
...
Both dictionaries and sets use hash-based lookup.
...
Use a `set` when you primarily care about:

```text
membership + uniqueness
```

Use a `dict` when you need:
...
| Key-value relationships | No       | Yes              |
| Typical use             | Sequence | Lookup / mapping |

* Modern Python dictionaries preserve insertion order.

---

# 119. Dictionary vs Tuple
...
| Main purpose    | Mutable sequence | Fixed sequence | Mapping    |

---

# 120. Dictionary vs Set

| Feature            | Dictionary      | Set                     |
| ------------------ | --------------- | ----------------------- |
| Stores             | Key-value pairs | Values                  |
| Unique keys/values | Keys unique     | Values unique           |
| Mutable            | Yes             | Yes                     |
| Hash-based         | Yes             | Yes                     |
| Membership         | Fast            | Fast                    |
| Indexing           | No              | No                      |
| Main use           | Mapping         | Uniqueness / membership |

---

# 121. Common Mistake: Using a List as a Key
...
Use a tuple if the collection should be a key:
...
# 122. Common Mistake: Assuming `get()` Raises an Error
...
unless a different default is supplied.

---

# 123. Common Mistake: Assuming `get()` Adds the Key
...
# 124. Common Mistake: Confusing Keys and Values
...
# 125. Common Mistake: Modifying While Iterating
...
# 126. Common Mistake: `dict[key]` on Optional Data
...
# 127. Common Mistake: Shallow Copy of Nested Data
...
when a true independent copy is required.

---

# 128. Common Mistake: Duplicate Keys
...
If you need multiple values associated with the same logical key, use a list:
...
or structure the data differently.

---

# 129. Common Mistake: Treating a Dictionary Like a List
...
because `0` is interpreted as a key.
...
# 130. Common Pattern: Lookup Table

Dictionaries are excellent for lookup tables.
...
# 131. Common Pattern: Configuration

Dictionaries are often used for configuration:
...
This structure is extremely common in APIs, JSON, ETL work, and data processing.

---

# 133. Common Pattern: Indexing Records by ID
...
returns Ivan's record directly.

This is an important pattern when working with data.

---

# 134. Common Pattern: Group Records by a Field
...
# 135. Common Pattern: Count Records by Category
...
# 136. Common Pattern: Build a Dictionary from Records
...
Dictionaries are closely related to keyword arguments.
...
# 140. Dictionary Unpacking in Function Calls
...
The keys in the dictionary become keyword argument names.

---

# 141. Dictionary Views and Iteration
...
# 142. Essential Dictionary Patterns to Memorize
